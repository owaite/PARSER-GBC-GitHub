...
#"2022_04_23",
# done: "2022_04_23"
date_list = c("2022_11_28","2023_06_21","2023_12_17")#,"2023_06_21")

for (i in 1:length(date_list)){
  
  date = date_list[i]

  dir = paste0(drive,":\\",name,"\\",L1_folder,"\\",date)
...
#if not empty, find trees with multiple polygons
unique(pols_dup$treeID)
## Manually delete secondary polygons in arcgis, usually they are very small
...
pols = st_read("path to poly _duplicatesRemoved") %>% 
  filter(!st_is_empty(.)) %>% 
  #filter(Edited != 4) #%>% # can remove pols based on confidence/ ingress
  st_make_valid() %>% 
  st_cast("POLYGON")%>%
  st_crs(26910)
...
pols_dup = pols %>% 
  group_by(treeID, rep, family) %>% #Sams: Seedlot, Tree, Rep
  filter(n()>1)
pols_dup 
...
# date_list <- c("2023_06_21","2023_12_17")
for (i in 1:length(date_list)){
  ...
  if(date != "2022_04_23"){
    ...
    polys_to_las = function(chunk, zq = 0.75, polygons = pols) {
      ...
      las_df = las2@data %>%
        ...
        dplyr::ungroup() %>% 
        dplyr::mutate(treeID = if_else(Z > quantile(Z, 0.999) * zq, as.numeric(treeID), 0))
...
las_files <- list.files(paste0(drive,":\\",name,"\\",L1_folder,"\\",date,"\\10_CROWNS_clean"), pattern = "\\.laz$", full.names = TRUE)
#above folder is 10_CROWNS_clean_treeID_noNULL for 2022_04_24
#trees that have thrown alpha shape errors: 

error_tags <- c(2753, 631, 863) # 2022_04_23 Canoe
error_tags <-c(956, 1267, 631, 863) #2022_11_28 Canoe
error_tags <- c(2753, 631, 863) #2023_06_21
