...
## (1) projected to nad83 and offset in lastools to ease initial manual alignmentin CC
...
#list of dates of data acquisitions
...
for (i in 1:length(date_list)){
  ...
  site = "Fdc_PR_Canoe" #older version also had a "name" variable that was a short form of the site ie "Canoe"
  ...
  ##############################################################################
  #(1)# Create 10cm DTMs (Ditgital Terrain Model) - ground identified in DJI terra - thinned to 1cm below
  ##############################################################################
...
##############################################################################
#(3)# Read in edited crown polygons for metric calculation: 
##############################################################################
...
## Manually delete secondary polygons in arcgis, usually they are very small reminants of the editing process
...
for (i in 1:length(date_list)){
  ...
  date = date_list[i]
  print(date)
  dir = paste0(drive,":\\",site,"\\",L1_folder,"\\",date)
  ...
  ##############################################################################
  #(6)# Clip merged cloud to individual trees and "clean" the tree point clouds
  ##############################################################################
  ...
  ## CLEANING crowns
  # function to drop remnant treeID values
...
##############################################################################
#(7)# Key Metrics function: looping through indv las files to add tag
##############################################################################
future:::ClusterRegistry("stop")
...
error_tags <- c()#initiate this empty vector
#as the for loop runs it may encounter an error. Keep a note of the error and the tag ID:
# i.e.: tag: 1971
        # Error in alphashape3d::ashape3d(x = a3d, alpha = Inf, pert = TRUE, eps = 1e-09) : 
        # NA/NaN/Inf in foreign function call (arg 2), add tag number into the error_tags vector and rerun the num = 0 below
        # then re run the for loop, this will skip this tree so you can go back later and troubleshoot the error
