...
PlotLetter = "A"
name = paste0("Rainbow_Plot",PlotLetter)
site = paste0("CW_PR_Rainbow",PlotLetter)
L1_folder = "L1_metrics" #Canoe_L1"
#"2022_04_23",
# done: "2022_04_23"
date_list = c("2022_11_20","2023_06_22","2023_12_17")

for (i in 1:length(date_list)){
  
  date = date_list[i]
  
  dir = paste0("A:\\WaiteOlivia_2\\Rainbow_L1\\",date,"\\Plot",PlotLetter)
...
pols_dir <- "A:\\WaiteOlivia_2\\Rainbow_L1\\2023_12_17_A\\05_ALIGNED_ALS\\pols\\Edited_CHM__06_masked_RGB_H20T_thres45percent_2022_06_07_NAfilledminw3_medw3_gauss_sig_01w3_z75_crowns_2_AbsIncluded_ND.shp"

pols_dir <- "A:\\WaiteOlivia_2\\Rainbow_L1\\2023_12_17\\PlotB\\pols\\CHM_06_masked_RGB_H20T_thres45percent_2022_06_07_NAfilledminw3_medw3_gauss_sig_01w3_z75_crowns_2_EDITED_legend_ND.shp"
pols = st_read(pols_dir, crs = 26910) %>% 
  filter(!st_is_empty(.)) %>% 
  # filter(Edited != "4") #%>%
  st_make_valid() %>% 
  st_cast("POLYGON")
...
pols_dup = pols %>% 
  group_by(treeID, REP, FAM) %>% #Sams: Seedlot, Tree, Rep
  filter(n()>1)
pols_dup 
...
pols$tag_1 = paste0(pols$ROW, 1,pols$COL) #making tag for rainbow
pols$tag = paste0(pols$ROW,pols$COL) #making tag for rainbow
pols$fam = pols$FAM
pols$rep = pols$REP
...
# date_list <- c("2022_04_22","2022_11_20","2023_06_22")
date_list <- c("2023_12_17")#"2022_11_20",2023_06_22
...
for (i in 1:length(date_list)){
  ...
  print("making crowns")
  opt_output_files(MERGED) = paste0(dir, "\\09_CROWNS\\", name, "_fam{fam}_rep{rep}_tag{tag}_row{ROW}_col{COL}_treeID{treeID}") #changed Seedlot to fam, changed tree to treeID
...
las_files <- list.files(paste0("A:\\WaiteOlivia_2\\Rainbow_L1\\",date,"\\Plot",PlotLetter,"\\10_CROWNS_clean"), pattern = "\\.laz$", full.names = TRUE)
#above folder is 10_CROWNS_clean_treeID_noNULL for 2022_04_24
#trees that have thrown alpha shape errors: 

error_tags <- c(369) #2022_11_20, 2023_06_20 RA
error_tags <- c()
