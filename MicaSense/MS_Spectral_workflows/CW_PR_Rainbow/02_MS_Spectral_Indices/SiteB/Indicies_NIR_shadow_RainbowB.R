...
RB_crowns <-"A:\\WaiteOlivia_2\\Rainbow_L1\\pols_B\\CHM_06_masked_RGB_H20T_thres45percent_2022_06_07_NAfilledminw3_medw3_gauss_sig_01w3_z75_crowns_2_EDITED_legend_ND.shp"

# reading in crown polygons
RB_pols_read = st_read(RB_crowns, crs = 26910) %>% 
  filter(!st_is_empty(.)) 

unique(sf::st_dimension(RB_pols_read))
#calculating area and adding a buffer
RB_pols_buf <- RB_pols_read %>%
  mutate(Area = st_area(RB_pols_read)) %>%
  st_buffer(dist = -.05)%>%
  filter(!st_is_empty(.)) 
#vect()

unique(sf::st_dimension(RB_pols_buf))

#calculating area of buffered crown
RB_pols_spat <- RB_pols_buf %>%
  mutate(Area_buffer5cm = st_area(RB_pols_buf))

colnames(RB_pols_spat)
class(RB_pols_spat)
...
RB_pols <- RB_pols_spat
class(RB_pols)
updated_metrics_folder <- "Updated_CSV_Index_NIR_Shadow"

RB_pols
RB_pols$tag = paste(RB_pols$ROW, RB_pols$COL, sep = "") #making tag for rainbow
RB_pols
unique(sf::st_dimension(RB_pols))
colnames(RB_pols)

for (x in 1:length(date_list)){
  print(date_list[x])
  dir <- paste0("A:\\WaiteOlivia_2\\Cw_PR_Rainbow_SiteB\\2_Processed_Work\\Flights\\",date_list[x],"\\2_Inputs\\")
  print(dir)
  dir_D <- paste0("A:\\WaiteOlivia_2\\Cw_PR_Rainbow_SiteB\\2_Processed_Work\\Flights\\",date_list[x])
  ...
  #mask NIR shadow to RB_pols:
  shadow_mask <- terra::mask(shadow_mask, RB_pols)
  ...
  ## mask raster to RB_pols
  ms_mask <- terra::mask(ms_mask, RB_pols)
  ...
  df_spectral_mean = exact_extract(rast_all, RB_pols, fun = "mean", append_cols = c("treeID","Edited","Edited_OW","random_num","tag","REP","COL","ROW","GenSam","Area_buffer5cm","Area"))
  print("calculating cell count")
  df_count = exact_extract(ms_mask[[1]], RB_pols, fun = "count", progress = TRUE, append_cols = c("treeID","Edited","Edited_OW","random_num","tag","REP","COL","ROW","GenSam","Area_buffer5cm","Area"))
  
  df_spectral_mean_count <- merge(df_spectral_mean,df_count, by = c("treeID","Edited","Edited_OW","random_num","tag","REP","COL","ROW","GenSam","Area_buffer5cm","Area"))
  ...
  df_spectral_median = exact_extract(rast_all, RB_pols, fun = "median", append_cols = c("treeID","Edited","Edited_OW","random_num","tag","REP","COL","ROW","GenSam","Area_buffer5cm","Area"))
  df_spectral_median_count <- merge(df_spectral_median,df_count, by = c("treeID","Edited","Edited_OW","random_num","tag","REP","COL","ROW","GenSam","Area_buffer5cm","Area"))
