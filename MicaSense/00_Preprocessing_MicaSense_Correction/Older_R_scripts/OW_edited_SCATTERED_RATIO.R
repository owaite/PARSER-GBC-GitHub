...
dir_final = "C:\\Users\\owaite\\Sync\\PARSER_UBC_DATA\\Fdc_PR_Canoe\\Flights"
...
for (x in #seq_along(date_list)
  ...
  for (j in 1:10){
    ...
    if (j<= 5){
      pics = list.files(paste0(dir, "\\",date_list[x],"\\1_Data\\Micasense_Cleaned_org_save\\MS_RED\\"), 
                            pattern = c(paste0("IMG_...._", j, ".tif")), recursive = TRUE, 
                            full.names = TRUE)
      
      pics_root = pics
    }
    if (j>= 6){
      pics = list.files(paste0(dir, "\\",date_list[x],"\\1_Data\\Micasense_Cleaned_org_save\\MS_BLUE\\"), 
                        pattern = c(paste0("IMG_...._", j, ".tif")), recursive = TRUE, 
                        full.names = TRUE)
      pics_root = pics
    }
    ...
    for (i in 1:length(pics)){
      ...
      if (substr(pic,11,11) == "0") {
        band = substr(pics[i],10,11)
      } else {
    ...
    saveRDS(exif_df, paste0(dir, "\\CSV\\Corrected_values\\df_", date_list[x], "_", j, ".rds"))
  } 
}
...
xmp_all_ssa = read_rds(paste0(dir, "\\CSV\\Corrected_values\\XMP_with_SSA.rds")) %>% 
  ...
  mutate(GPSLatitude = scale(as.numeric(GPSLatitude)),
         GPSLongitude = scale(as.numeric(GPSLongitude)),
...
(xmp_vals_choose %>% 
    ...
    geom_hline(yintercept = 0.489, color = "grey10") +
...
(xmp_vals_choose %>% 
    ...
    ggplot(aes(x = irr_diff_max, y = irr_cv_max, group = BandName, color = BandName)) +
    geom_point(size = 1.5) +
    geom_point(data = filter(xmp_vals_choose, choose_flag == 1), 
               size = 1.5,
               color = "red4") +
