...
date_range <- "2022_07_06-2022_08_24" #"2022_07_06_to_2022_08_24"

ext_save <- "_org_save" #make either "" for "Micasense_Cleaned" or "_org_save" for "Micasense_Cleaned_org_save"


#################################################################################
#(0) Transferring Panels folder to the main directory with MS original images
################################################################################
...
for (x in seq_along(date_list)) {
  with_progress({
    p <- progressor(steps = length(1:10))
    exif_df_list <- furrr::future_map(1:10,function(j){
    pics <- list.files(paste0(dir, date_list[x], "\\1_Data\\Micasense_Cleaned", ext_save),
                       pattern = c(paste0("IMG_...._", j, ".tif"), paste0("IMG_...._", j, "_", ".tif")),
    ...
    for (i in seq_along(pics)) {
      ...
      pan_flag <- ifelse(pic %in% panel_dirs, 1, 0)
      ...
      # Update progress bar
     # progress_update(i / length(pics))
    }
    
    saveRDS(exif_df, paste0(dir, date_list[x], "\\1_Data\\CSV\\Corrected_values\\df_", date_list[x], "_", j, ".rds"))
    
    return(exif_df)  # Return the exif_df for this band
  },.progress = TRUE)})
  
  if (!dir.exists(paste0(dir, date_list[x], "\\1_Data\\CSV\\"))) {
    dir.create(paste0(dir, date_list[x], "\\1_Data\\CSV\\"))
  }
  
  if (!dir.exists(paste0(dir, date_list[x], "\\1_Data\\CSV\\Corrected_values\\"))) {
    dir.create(paste0(dir, date_list[x], "\\1_Data\\CSV\\Corrected_values\\"))
  }
  
  # Combine the exif_df from the list of results
  exif_df <- do.call(rbind, exif_df_list)
  
  saveRDS(exif_df, paste0(dir, date_list[x], "\\1_Data\\CSV\\Corrected_values\\df_", date_list[x], ".rds"))
}



### Below IS paralized: ---------------------------------------------------------
# Export necessary variables to the parallel workers
clusterExport(cl, varlist = c("dir", "date_list", "ext_save"))

# Load required packages and functions within the parallel workers
clusterEvalQ(cl, {
  library(magrittr)
  library(exifr)
  library(tidyverse)
  ...
  # Load the package that contains read_exif
})
# Your existing code with parallelization for bands using foreach
for (x in seq_along(date_list)) {
  exif_df_list <- future_map(1:10, function(j) {
    pics <- list.files(paste0(dir, date_list[x], "\\1_Data\\Micasense_Cleaned", ext_save),
                       pattern = c(paste0("IMG_...._", j, ".tif"), paste0("IMG_...._", j, "_", ".tif")),
                       recursive = TRUE, full.names = TRUE)
...
for (x in seq_along(date_list)){
  for (j in 1:10){ # bands 1 through 10 
    ...
    #                              pattern = c("IMG_...",".tif$"), recursive = TRUE, 
    #                              full.names = FALSE)#list of panel image names
    ...
    for (i in 1:length(pics)){
      ...
      if (substr(pic_root,11,11) == "0") { #distinguishes between _1 and _10
        band = substr(pic_root[i],10,11) # for _10
      } else {
        band = substr(pic_root,10,10)} # for _1
...
panel_directory <- "I:\\PARSER_Ext\\Fdc_PR_Canoe\\Flights\\2022_07_06\\1_Data\\Micasense_Cleaned_org_save/Panels"
keyword = "Panel"
panel_dirs <- pics[grep(keyword, pics)]
...
xmp_all_new <- xmp_all %>%
  mutate(panel_flag = ifelse( SourceFile %in% SourceFile[grep(keyword, SourceFile)], 1, 0))
...
xmp_all_ssa = read_rds(paste0(dir, "CSV\\Corrected_values\\XMP_", date_range,"_with_SSA.rds")) %>% # SET PATH
  ...
  mutate(Date = sub(".+Flights\\\\(.+?)\\\\1_Data.+", "\\1", Directory), # the Date from the directory
         subdirectory = grepl("Panels", Directory), # folder where the images are (all previous slashes are backslashes, first /)
         panel_flag = if_else(subdirectory == TRUE, 1, 0)) %>% # the only use of subdirectory is to check if images are in PANELS
...
#left when it was runnning the above
df = mod_frame %>% 
...
xmp_panels = xmp_corrected %>% 
  ...
  mutate(root = str_split_i(Directory, "\\/", i = 1),
         ...
         mask_path = paste0("D:\\Fdc_PR_Canoe\\Flights\\",Date,"\\2_Inputs\\metashape\\MASKS\\",img_name, "_mask.png"))
